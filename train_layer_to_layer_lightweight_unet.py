...
from unet import LightweightUNetLayer2Layer, SuperLightweightUNetLayer2Layer, SuperDuperLightweightUNetLayer2Layer
...
def train_model(
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s: %(levelname)s: %(message)s',
                        filename=os.path.join(str(Path(args.dir_checkpoint) / experiment.id), 'log.log'))
...
def get_args():
    ...
    parser.add_argument('--output_dir', type=str, default='outs/')
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s: %(levelname)s: %(message)s',
                        filename=os.path.join(args.output_dir, f'log_{args.layername_in}-to-{args.layername_out}.log'))
    ...
    if args.unet_type == 'LightweightUNetLayer2Layer':
