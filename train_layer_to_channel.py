...
from evaluate_layer_to_layer import evaluate_acc_metrics
...
def train_model(
        ...
        layername_out_specific_channel: int = 0,
        epochs: int = 5,
        batch_size: int = 1,
    ...
    for epoch in range(1, epochs + 1):
            ...
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if args.criterion == 'ce-loss+dice-loss':
                        ...
                        if model.n_classes == 1:
                            ce_loss = ce_loss_criterion(masks_pred.squeeze(1), true_masks.float())
                            loss = get_criterion(args)(masks_pred.squeeze(1), true_masks.float())
                        else:
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        val_metrics = evaluate_acc_metrics(model, base_classifier, val_loader, device, amp, FEATURES_CACHE_DICT, layername_out_specific_channel)
        ...
        if save_checkpoint:
            ...
            os.makedirs(str(Path(args.dir_checkpoint) / experiment.id ), exist_ok=True)
            torch.save(state_dict, str(Path(args.dir_checkpoint) / experiment.id / 'checkpoint_epoch{}.pth'.format(epoch)))
...
if __name__ == '__main__':
    ...
    try:
        train_model(
            ...
            layername_out_specific_channel=args.layername_out_specific_channel,
        )
